...
This image uses the debian based nginx image.
...
To run tests, you need to prepare the docker image to test which must be tagged `pinidh/nginx-proxy:test`:

    docker build -t pinidh/nginx-proxy:test .  # build the Debian variant image

and call the [test/pytest.sh](test/pytest.sh) script.

Then build the Alpine variant of the image:

    docker build -f Dockerfile.alpine -t pinidh/nginx-proxy:test .  # build the Alpline variant image
